...
const TABLE_HEAD = [
  ...
  { id: 'Challan', label: 'Sent', align: 'center', width: 140 },
...
function applyFilter({
  ...
  if (filterName) {
    inputData = inputData.filter(
      (invoice) =>
        invoice.invoiceNo.toLowerCase().indexOf(filterName.toLowerCase()) !== -1 ||
        invoice.invoiceTo.clientName.toLowerCase().indexOf(filterName.toLowerCase()) !== -1
    );
  }
  ...
  if (filterStartDate && filterEndDate) {
    inputData = inputData.filter(
      (invoice) =>
        fTimestamp(invoice.invoiceDate) >= fTimestamp(filterStartDate) &&
        fTimestamp(invoice.invoiceDate) <= fTimestamp(filterEndDate)
    );
  }
